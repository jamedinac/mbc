...
Generated by `ClusterGenerationService` when the `--profile` (`-p`) flag is enabled. The report is saved as `profile_metrics.txt` in the working directory and contains input vs. clustered dimensions, performance metrics, and a full list of retained gene and sample IDs.
