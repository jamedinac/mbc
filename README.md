...
- `-k`, `--clusters`: Number of clusters (k) for the genes that pass filtering. The final output will contain **K+1 clusters**: the K algorithmically-formed clusters plus one additional cluster for all genes that were filtered out. **Default**: `10`
...
- `-p`, `--profile`: Flag to enable profiling. Records execution time, peak memory usage, input vs. clustered gene/sample counts, and lists all retained gene and sample IDs in `profile_metrics.tsv`.
...
### Options
- `-nd`, `--normalized-data`: Path to the normalized gene expression matrix (CSV). This file is generated by `ClusterGenerationService` and is required for calculating internal metrics (like WCSS and Silhouette).
- `-d`, `--distance`: Distance metric (`correlation`, `euclidean`, `jensenshannon`). **Default**: `correlation`
...
The output contains **K+1 columns**: K clusters from the clustering algorithm plus one additional column for genes that were filtered out during preprocessing. Genes that passed filtering have their cluster membership values in columns 1–K and `0.0` in the last column. Genes that were filtered out have `0.0` in columns 1–K and `1.0` in the last column.

**Example `clusters.txt`** (K=2, 3 columns total)
```tsv
G1	1.0	0.0	0.0	
G2	0.0	1.0	0.0	
G3	1.0	0.0	0.0	
G_FILTERED1	0.0	0.0	1.0	
G_FILTERED2	0.0	0.0	1.0	
```

### 3. Benchmark Output (TSV)

Generated by `ClusterBenchmarkService`. By default, the output file shares the base name of the input clusters file, suffixed with `_benchmarks.tsv`.

The output of `ClusterBenchmarkService` provides a summary of all metrics and detailed gene-level statistics where applicable in a structured three-column format: `benchmark`, `geneid`, and `value`.

**Example `benchmarks.tsv`**
```tsv
benchmark	geneid	value
jaccard	global	0.4863484382027565
accuracy	global	0.6821392532795156
adjusted_rand_index	global	0.405038972595072
nmi	global	0.5129228589506647
silhouette	global	0.780270123
silhouette	GENE1	0.812345
silhouette	GENE2	0.795432
silhouette	GENE3	-0.123456
silhouette	GENE4	0.654321
```

### 4. Profile Report (TSV)

Generated by `ClusterGenerationService` when the `--profile` (`-p`) flag is enabled. The report is saved as `profile_metrics.tsv` in the working directory and contains input vs. clustered dimensions, performance metrics, and a full list of retained gene IDs, formatted as a three-column TSV file: `Category`, `Metric`, and `Value`.
...
profiling_metrics	clustered_genes	476
profiling_metrics	time_series	12
profiling_metrics	execution_time_seconds	12.0006
profiling_metrics	peak_heap_memory_mb	60.35
filtered_out_genes	GENE301	ZERO_FILTER
filtered_out_genes	GENE302	ZERO_FILTER
filtered_out_genes	GENE303	VARIANCE_FILTER
filtered_out_genes	GENE500	SIGNIFICANCE_FILTER
```
