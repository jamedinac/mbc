...
public class KMeansAlgorithm implements IClusteringAlgorithm {
    ...
    double[][] generateCentroids(GeneExpressionData geneExpressionData) {
        ...
        boolean[] selectedGenes = new boolean[numberOfGenes];

        for (int i = 0; i < this.k; i++) {
            int geneIndex;
            do {
                geneIndex = RandomGenerator.uniformRandomInt(numberOfGenes);
            } while (selectedGenes[geneIndex]);

            selectedGenes[geneIndex] = true;
            centroids[i] = geneExpressionData.getGeneProfile(geneIndex);
        }
